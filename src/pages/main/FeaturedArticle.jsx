      ...
      <div className="col-span-1 text-gray-700">
        ...
        <div className="flex items-center text-sm">
          <span className="mr-4 flex items-center gap-1">
            <FaRegHeart />
            {article.author.likes}
          </span>
          <span className="flex items-center gap-1">
            <FaRegComment /> {article.author.comments}
          </span>
        </div>
      </div>
        ...
        <div className="flex items-center mb-4">
          ...
          <div>
            ...
            <span className="text-sm text-gray-500 ml-2">
              {article.author.time} · {article.author.readTime}
            </span>
          </div>
        </div>
        <p className="text-gray-700 mb-4">
          {article.content}
          <a href={article.link} className="text-red-500">
            {' '}
            {article.linkText}
          </a>
          . Các dự án dưới đây được mình ngẫu nhiên lựa chọn để đăng chứ không mang tính xếp hạng
          các bạn nhé.
        </p>
