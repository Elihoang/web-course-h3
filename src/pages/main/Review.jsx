...
const Review = ({ courseId, reviews, isEnrolled, onReviewSubmitted }) => {
  ...
  // Sync reviews prop with local state
  useEffect(() => {
    setDisplayReviews(reviews || []);
  }, [reviews]);

  // Fetch reviews and check for existing user review
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await getReviewsByCourseId(courseId);
        if (response && Array.isArray(response)) {
          setDisplayReviews(response);
  ...
  return (
    <div className="mt-8">
      {/* Form đánh giá chỉ hiển thị nếu người dùng đã đăng ký và đang đăng nhập */}
      {isEnrolled && (
        <div>
          <h3 className="text-xl font-bold mb-4">
            {existingReview ? 'Cập nhật đánh giá của bạn' : 'Viết đánh giá của bạn'}
          </h3>
